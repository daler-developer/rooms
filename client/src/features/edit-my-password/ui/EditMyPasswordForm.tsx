...
import { PASSWORD_FORM_GET_ME, EDIT_MY_PASSWORD } from "../gql";
...
const EditMyPasswordForm = () => {
  ...
  const getMeQuery = useQuery(PASSWORD_FORM_GET_ME);

  const [editMyPassword, { loading: isEditingMyPassword }] = useMutation(EDIT_MY_PASSWORD);
  ...
  return (
    <>
      <div className="flex items-center gap-2 justify-between">
        <span>{new Array(getMeQuery.data!.me.passwordLength).fill("*").join("")}</span>
