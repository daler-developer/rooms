...
type Props = {
  message: Flatten<GetRoomQuery["room"]["messages"]["data"]>;
  intersectionAnchor: RefObject<HTMLElement>;
};

const Message = ({ message }: Props) => {
  const { selectedMessages, addSelectedMessage, removeSelectedMessage } = useRoomChatStore();
  ...
  const isSentByMe = data!.me.id === message.sender.id;
  ...
  const formattedCreatedAt = useMemo(() => {
    return `${String(dayjs(message.sentAt).hour()).padStart(2, "0")}:${String(dayjs(message.sentAt).minute()).padStart(2, "0")}`;
  }, [message]);
  ...
  return (
    <BaseMessage
      ...
      bottomRight={<span className="text-[11px]">{formattedCreatedAt}</span>}
      bottomLeft={<MessageViews message={message} />}
